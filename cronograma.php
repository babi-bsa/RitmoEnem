...
// Inicializa variáveis para evitar avisos em carregamento inicial da página
$materiasInput = [];
$dificuldadesInput = [];
$errors = [];
$scheduleOutput = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validações para gerar um cronograma viável
    ...
    // Cálculo do intervalo de horas disponíveis como horas cheias válidas
    list($startHourPart, $startMinutePart) = explode(':', $horarioInicio);
    list($endHourPart, $endMinutePart) = explode(':', $horarioFim);
    $startHour = (int)$startHourPart;
    $startMinute = (int)$startMinutePart;
    $endHour = (int)$endHourPart;
    $endMinute = (int)$endMinutePart;

    $startSlot = $startMinute > 0 ? $startHour + 1 : $startHour;
    $endSlot = $endMinute > 0 ? $endHour : $endHour - 1;
    $totalSlotsAvailableInterval = max(0, $endSlot - $startSlot + 1);
    ...
    if (empty($errors)) {
        ...
        // Alocação das matérias no cronograma dia a dia com distribuição mais uniforme
        foreach ($dias as $dia) {
            $allocatedHoursToday = 0;

            // Reordena as matérias pelo número de horas restantes no início do dia,
            // para garantir que a distribuição entre dias se mantenha equilibrada.
            usort($materiasParaAlocar, function($a, $b) {
                if ($b['horas_restantes'] === $a['horas_restantes']) {
                    return $b['peso'] <=> $a['peso'];
                }
                return $b['horas_restantes'] <=> $a['horas_restantes'];
            });

            $subjectOrder = array_keys($materiasParaAlocar);
            $subjectPointer = 0;
            $numMaterias = count($subjectOrder);

            for ($h = $startSlot; $h <= $endSlot && $allocatedHoursToday < $horas; $h++) {
                if (empty($numMaterias)) {
                    break;
                }

                $attempts = 0;
                $foundIndex = null;
                while ($attempts < $numMaterias) {
                    $candidateIndex = $subjectOrder[$subjectPointer];
                    if ($materiasParaAlocar[$candidateIndex]['horas_restantes'] > 0) {
                        $foundIndex = $candidateIndex;
                        break;
                    }
                    $subjectPointer = ($subjectPointer + 1) % $numMaterias;
                    $attempts++;
                }

                if ($foundIndex === null) {
                    break; // Não há mais horas a alocar
                }

                $currentMateria = $materiasParaAlocar[$foundIndex];
                $schedule[$dia][$h] = [
                    'name' => $currentMateria['name'],
                    'level' => $currentMateria['level']
                ];
                $materiasParaAlocar[$foundIndex]['horas_restantes']--;
                $allocatedHoursToday++;

                // Avança para o próximo assunto, mantendo o ciclo
                $subjectPointer = ($subjectPointer + 1) % $numMaterias;
            }
        }
...
HTML;
                    ...
                    } else { // Se não houver submissão ou não houver erros, exibe o primeiro campo vazio
                    ?>
                    <div class="form-row mb-2">
